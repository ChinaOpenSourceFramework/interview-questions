...
public class RedisConfig {
	...
	/**
	 * 从application.yml取得database的数据库.
	 */
	...
	/**
	 * 从application.yml取得redis的password的密码.
	 */
	...
	/**
	 * 关于JedisConnectionFactory的配置： 1.
	 * 主机与端口号:在配置体系中由RedisStandaloneConfiguration负责设置,在JedisConnectionFactory中有个standaloneConfig字段默认设置了"localhost:6379"的地址
	 * 2.
	 * 用户名与密码:在配置体系中由RedisSentinelConfiguration负责配置,但redis默认可以无需用户登录,所以要配置用户名与密码需要调用JedisConnectionFactory(RedisSentinelConfiguration
	 * sentinelConfig)的构造方法 3.
	 * 其他配置:redis的其他配置选项由三层架构构成,分别为:BaseObjectPoolConfig->GenericObjectPoolConfig->JedisPoolConfig
	 ...
	 * 如果用户自定义了JedisPoolConfig,则MutableJedisClientConfiguration会调用setPoolConfig(GenericObjectPoolConfig
	 * poolConfig)方法进行处理,可以注意到,这里的poolconfig实际上是一个GenericObjectPoolConfig 4. 总结:
	 * 要是不进行配置,直接调用JedisConnectionFactory的无参构造即可;
	 * 如果要设置密码,实例化RedisSentinelConfiguration进行配置;
	 * 如果要设置主机,实例化RedisStandaloneConfiguration进行配置 如果要设置其他项目,实例化JedisPoolConfig进行配置
	 * 
	 * @return
	 */
